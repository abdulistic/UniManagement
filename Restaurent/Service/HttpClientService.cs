...
namespace Restaurent.Service
{
    ...
    public class HttpClientService : IHttpClientService
    {
        ...
        public async Task<string> PostAsyncMultiPart<T>(string uri, T item, string token = "")
        {
            var requestContent = new MultipartFormDataContent();

            requestContent.Add(new StringContent(JsonConvert.SerializeObject(item), Encoding.UTF8, "application/json"));
